...
// 将静态变量改为非静态，以便外部访问（用于停止功能）
std::atomic_bool have_quit(false);
io_context io_svc;                                                                   // handle I/O events
executor_work_guard<io_context::executor_type>* p_work_guard = nullptr;    // 使用指针以便重新创建
boost::thread_group vpn_thread_pool;  // 重命名避免与boost::asio::thread_pool冲突
signal_set signals(io_svc, SIGINT, SIGTERM);
// 保存当前路由IP，用于路由管理
static std::string current_route_ip;
...
bool init_wintun_adapter(const char* ip, int prefix, const char* target_ip)
{
    ...
    if (!tun_adapter)
    {
        ...
        if (!tun_adapter)
        {
            ...
            if (error == 5)
            {
                std::cerr << "创建WinTUN适配器失败：需要管理员权限！" << std::endl;
                std::cerr << "请以管理员身份运行此程序。" << std::endl;
                std::cerr << "错误代码: " << error << " (ERROR_ACCESS_DENIED)" << std::endl;
            }
            else
            {
                std::cerr << "Failed to create/open WinTUN adapter. Error: " << error << std::endl;
            }
            return false;
        }
    }
    ...
    if (target_ip && strlen(target_ip) > 0) {
        ...
        current_route_ip = target_ip;  // 保存路由IP
...
{
    if (!tun_adapter || tun_luid.Value == 0) {
        std::cerr << "TUN adapter not initialized" << std::endl;
        return false;
    }
    ...
    FlushIpPathTable(AF_INET);
    current_route_ip = target_ip;  // 保存路由IP
    std::cout << "Route added for " << target_ip << std::endl;
    return true;
}
...
{
    if (tun_luid.Value == 0) {
        std::cerr << "TUN adapter LUID not available" << std::endl;
        return false;
    }
    ...
    DWORD result = DeleteIpForwardEntry2(&route);
    if (result != ERROR_SUCCESS && result != ERROR_NOT_FOUND)
    {
        std::cerr << "Failed to delete route for " << target_ip << " Error: " << result << std::endl;
        return false;
    }
    FlushIpPathTable(AF_INET);
    std::cout << "Route removed for " << target_ip << std::endl;
    return true;
}
...
{
    if (current_route_ip.empty()) {
        return true;  // 没有路由需要删除
    }
    return remove_route(current_route_ip.c_str());
}
...
void vpn_client(const std::string& server_ip, int port, const std::string& tun_ip, const char* config_path)
{
    try
    {
        ...
        if (future.wait_for(std::chrono::seconds(3)) == std::future_status::timeout) {
            std::cerr << "[WARN] Thread pool join timeout" << std::endl;
        }
...
int start_vpn_client(const char* config_path, const char* route_ip)
{
    ...
    if (!config_path)
    {
        std::cerr << "参数错误：config_path 为空" << std::endl;
        return 0;
    }
    // route_ip 可以为空（只建立连接，不添加路由）
    std::ifstream config_file(config_path);
    if (!config_file)
    {
    ...
    // 重置退出标志和io_svc（支持多次启动）
    have_quit = false;
    io_svc.restart();
    
    // 创建新的work_guard
    p_work_guard = new executor_work_guard<io_context::executor_type>(io_svc.get_executor());

    // Initialize Winsock (只初始化一次)
    if (!wsa_initialized)
    {
        WSADATA wsa_data;
    ...
    // Initialize OpenSSL (只初始化一次，不要清理)
    ...
    if (!init_wintun_adapter(tun_ip.c_str(), 24, route_ip ? route_ip : nullptr))
    {
        CleanupWintun();
        delete p_work_guard;
        p_work_guard = nullptr;
    ...
    // 注意：不要调用 WSACleanup() 和 EVP_cleanup()
    // 因为这些是全局资源，应该在程序退出时才清理
    // 如果在这里清理，第二次启动时会失败
...
// 停止VPN客户端（供UI调用）
void stop_vpn_client_impl() {
    have_quit = true;
    if (p_work_guard) {
        p_work_guard->reset();
    }
    // 停止io_svc，允许run_one()退出
    if (!io_svc.stopped()) {
        io_svc.stop();
        // 取消所有未完成的异步操作
        io_svc.poll();
...
// main函数已排除，由UI主程序调用start_vpn_client
#if 0
