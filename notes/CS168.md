...
1. Narrow Waist: **IP**(Internet Protocol) is the only protocol at Layer 3 ![2-1](./picture/2-1.png)
   - all hosts and routers understands IP
   - unifies the Internet and enables federation
2. Demultiplexing at Layer 3: after receiving a packet, we need to konw which way to send it to higher-layer protocols.![2-2](./picture/2-2.png)
   - Add a new header filed that tells us what the next (higher) layer protocol is 
   - Allow the IP code to pass the rest of the packet to the appropriate L4 code ![2-3](./picture/2-3.png)
3. Demultiplexing at Layer 4:
   - Each running application on the computer is associated with a **port number**
   - When L4 receives a packet, it uses the port number to pass the packet to the corresponding application ![2-4](./picture/2-4.png)
![2-5](./picture/2-5.png)
4. About port number:
   - A private client (e.g. your computer) can use a randomly-generated port number
   - A public server (e.g. YouTube) must use a fixed, well-known port number ![2-6](./picture/2-6.png)
   - **Physical port**: The hole to plug a cable into. Exists in a a hardware
   - **Logical port**: A number identifying an application. Exists in software
5. Implementing Layers in the End Host: ![2-7](./picture/2-7.png)
   - Demultiplexing helps the operating system pass packet to the correct application ![2-8](./picture/2-8.png)
   - Logical ports identify the attachment poin between the application and the OS ![2-9](./picture/2-9.png)
