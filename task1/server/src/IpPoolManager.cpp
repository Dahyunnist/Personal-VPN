...
void IpPoolManager::generate_config_file(){
    try{
        ...
        nlohmann::json config = {
            {"server", {
                {"ip", get_vm_ip()},
                {"port", vpn_global::port}
            }},
            {"tun", {
                {"ip", available_ip},
                {"mask", vpn_config::TUN_MASK}
            }},
            {"certs", {
                {"client_crt", read_file(vpn_config::CLIENT_CRT_PATH)},
                {"client_key", read_file(vpn_config::CLIENT_KEY_PATH)},
                {"server_crt", read_file(vpn_config::CERTIFICATE_PATH)}
            }}
        };
