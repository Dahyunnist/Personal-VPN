...
{
   public:
    Session(ssl::stream<tcp::socket> socket, TunDevice& tun) : socket_(std::move(socket)), tun_(tun), active_(true) {}
   ...
   private:
    ...
    void tun_read_loop()
    {
        ...
        while (running_)
        {
            ...
            // 将数据发送给所有活跃的客户端
            asio::post(io_context_, [this, buffer, n]() { broadcast_to_clients(buffer.data(), n); });
        }
